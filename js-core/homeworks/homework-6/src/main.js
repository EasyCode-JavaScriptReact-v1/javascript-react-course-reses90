...
function reverseEachWord(str) {
    ...
    let strTransformToArray = str.split(' ');
    strTransformToArray.forEach(function(value, index, arr) {
        let reversingEachWord = value.split('').reverse().join('');
        newArray.push(reversingEachWord);
    });
    let newArrayTransformToString = newArray.join(' ');
    return newArrayTransformToString;
}
...
function wordCounter(sentence) {
    const sentenceTransformToArray = sentence.split(' ');
    const sortSentenceArray = sentenceTransformToArray.sort();
    let objectWithWords = {};
    sortSentenceArray.forEach(function(word, index, arr) {
        objectWithWords[word] = objectWithWords[word] + 1 || 1;
    });
...
function createHashTags(arr) {
    const objectWithIdAndCompanies = {};
    arr.forEach(function(value, index, arr) {
        objectWithIdAndCompanies[value._id] = value.company;
    });
    return objectWithIdAndCompanies;
}
